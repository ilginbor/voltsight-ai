...
METRICS_PATH = (
    ...
    / "data"
    / "processed"
    / "ankara_activity_category_context_metrics.csv"
)
...
FOLD_METRICS_PATH = (
    ...
    / "data"
    / "processed"
    / "ankara_activity_category_context_fold_metrics.csv"
)
...
OOF_PATH = (
    ...
    / "data"
    / "processed"
    / "ankara_activity_category_context_oof_predictions.csv"
)

PLOT_PATH = (
    PROJECT_ROOT
    / "docs"
    / "ankara_activity_category_context.png"
)

SUMMARY_PATH = (
    PROJECT_ROOT
    / "docs"
    / "ankara_activity_category_context_summary.md"
)
...
# The audit found poi_count_within_1000m and
# transport_activity_within_1000m at Spearman ~0.90. This combined set keeps
# the broader 2-km total context but drops the redundant 1-km total count.
...
FEATURE_SET_LABELS = {
    ...
    "normalized_12_plus_category_1km_context": (
        "Normalized 12 + category 1 km context"
    ),
    "normalized_12_plus_parsimonious_mixed_context": (
        "Normalized 12 + parsimonious mixed activity"
    ),
}
...
# sqrt(12) and sqrt(15) both floor to 3 in the historical RF baseline.
# Fixing three candidate predictors per split keeps the RF split-search budget
# unchanged when testing 16- and 18-feature category-context sets.
RANDOM_FOREST_MAX_FEATURES = 3


def validate_feature_sets() -> None:
    """Validate the fixed target-agnostic category-context design."""
    ...
    expected_counts = {
        "normalized_12": 12,
        "normalized_12_plus_total_activity_context": 15,
        "normalized_12_plus_category_1km_context": 16,
        "normalized_12_plus_parsimonious_mixed_context": 18,
    }
    ...
    if (
        ...
        raise ValueError(
            "The parsimonious mixed context must exclude the redundant "
            "1-km total POI count."
        )
    ...
    if (
        ...
        raise ValueError(
            "The parsimonious mixed context must retain transport activity."
        )
...
def validate_activity_frame(
    ...
    """Validate all activity predictors used by this experiment."""
    ...
    if (
        ...
        raise ValueError(
            "1-km total POI count cannot be below local total POI count."
        )
    ...
    if (
        ...
        raise ValueError(
            "2-km total POI count cannot be below 1-km total POI count."
        )

    for column in CATEGORY_1KM_CONTEXT:
        if (
            result[
                column
            ]
            ...
            raise ValueError(
                f"{column} cannot exceed the total 1-km POI count."
            )
...
def attach_activity_features(
    ...
    """Attach activity context to the existing spatial-CV baseline frame."""
...
def load_analysis_frame() -> pd.DataFrame:
    """Load the fixed baseline folds and attach activity context."""
...
def build_model(
    ...
    """Build one fixed estimator for this category-context sensitivity."""
...
def calculate_ranking_metrics(
    ...
    """Calculate rare-class ranking metrics."""
...
def add_incremental_deltas(
    ...
    """Add per-model deltas against normalized-12."""
...
def run_experiment(
    ...
    """Run all fixed category-context configurations."""
...
def create_plot(
    ...
    """Plot pooled AP for total and category activity contexts."""
...
def create_summary(
    ...
    """Write the category-context sensitivity summary."""
    ...
    summary = f"""# Ankara OSM Activity Category Context Sensitivity
...
def save_outputs(
    ...
    """Save activity category-context outputs."""
...
def print_results(
    ...
    """Print the main category-context metrics."""
...
def main() -> None:
    """Run the Ankara activity category-context sensitivity."""
    ...
    print(
        "VoltSight - Ankara OSM Activity Category Context Sensitivity"
    )
    ...
    print(
        "Ankara activity category-context sensitivity completed successfully."
    )
